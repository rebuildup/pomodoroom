...
} from "@/components/m3/OverviewProjectManager";
...
export default function ShellView() {
	...
	const [_guidanceRefreshNonce, setGuidanceRefreshNonce] = useState(0);
	...
	const readyTasks = useMemo(() => taskStore.getTasksByState("READY"), [taskStore]);
	const pausedTasks = useMemo(() => taskStore.getTasksByState("PAUSED"), [taskStore]);
	...
	const ambientCandidates = useMemo(() => {
		...
		const candidates: Array<
			Task & { reason: string; state: "PAUSED" | "READY"; autoScheduledStartAt: string }
		> = [];
		if (pausedTasks.length > 0) {
			candidates.push({
				...
				autoScheduledStartAt:
					pausedTasks[0].fixedStartAt || pausedTasks[0].windowStartAt || nextSlotTime,
			});
		}
		...
		if (candidates.length < 2) {
			...
			if (sameProjectTask) {
				candidates.push({
					...
					state: "READY",
					autoScheduledStartAt:
						sameProjectTask.fixedStartAt || sameProjectTask.windowStartAt || nextSlotTime,
				});
			}
		}
		...
		if (candidates.length < 2) {
			...
			if (highEnergyTask) {
				candidates.push({
					...
					state: "READY",
					autoScheduledStartAt:
						highEnergyTask.fixedStartAt || highEnergyTask.windowStartAt || nextSlotTime,
				});
			}
		}
		...
		if (candidates.length < 2) {
			...
			if (availableTask) {
				candidates.push({
					...
					state: "READY",
					autoScheduledStartAt:
						availableTask.fixedStartAt || availableTask.windowStartAt || nextSlotTime,
				});
			}
		}
	...
	useEffect(() => {
		...
		const runCalendarContextReset = async () => {
			...
			try {
				result = await invoke<{ calendar_ids: string[] }>(
					"cmd_google_calendar_get_selected_calendars",
				);
			} catch {
				selectedCalendarIds = [
					...new Set(calendar.events.map((event) => event.calendarId).filter(Boolean) as string[]),
				];
			}
			if (result) {
			...
			if (decision.action === "reset") {
				...
				downshiftFocusRampState(1, `calendar:${decision.cause.eventId}:${decision.cause.reason}`);
			}
	...
	const handleTaskOperation = useCallback(
		async (
			...
			try {
				...
				console.error(
					`[ShellView] Error executing task operation ${effectiveOperation} on task ${taskId}:`,
					errorMessage,
				);
				window.dispatchEvent(new CustomEvent("tasks:refresh"));
				throw error;
	 ...
	 */
	const handleDetailUpdateTask = useCallback(
	 ...
	 */
	const handleDetailTransitionTask = useCallback(
		(id: string, to: TaskState, operation?: string) => {
	 ...
	 */
	const handleDetailDeleteTask = useCallback(
		(id: string) => {
	 ...
	 */
	const handleCanTransition = useCallback(
		(id: string, to: TaskState) => {
	...
	const showCriticalStartIntervention = useCallback(
		...
		[statusSync, escalationBadges],
	);

	const handleRequestStartNotification = useCallback(
		(taskId: string) => {
			const task = taskStore.getTask(taskId);
			if (!task) return;
	...
	const handleRequestInterruptNotification = useCallback(
		(taskId: string) => {
			const task = taskStore.getTask(taskId);
	...
	const handleRequestPostponeNotification = useCallback(
		(taskId: string) => {
			const task = taskStore.getTask(taskId);
			if (!task) return;
	...
	const handleExecuteReference = useCallback(
		async (reference: { kind: string; value: string; label?: string }) => {
	...
	useEffect(() => {
		...
		if (delayMs <= 0) {
			...
			const dueStart =
				dueTask.fixedStartAt ?? dueTask.windowStartAt ?? dueTask.estimatedStartAt ?? "";
		...
		console.log(
			`[ShellView] Setting notification timer for ${delayMs}ms (${new Date(nextStartMs).toLocaleTimeString("ja-JP")})`,
		);
	...
	}, [taskStore.tasks, showCriticalStartIntervention]);
	...
	useMemo(() => {
		...
		return items.filter((task) => {
			const fields = [task.title, task.description ?? "", task.project ?? "", ...(task.tags ?? [])];
	...
	const statusTimelineSegments = useMemo(() => {
		...
		// Add calendar events
		const fromCalendar = getEventsForDate(calendar.events, todayDate)
			.map((e) => ({
	...
	const getTitle = () => {
		switch (activeDestination) {
			case "overview":
				return {
					title: "Overview",
					subtitle: isEmptyState ? "Add tasks to get started" : "Today at a glance",
				};
			case "tasks":
	...
	const renderContent = () => {
		switch (activeDestination) {
			case "tasks":
				return (
					<TasksView
						initialAction={pendingTasksAction}
						onActionHandled={() => setPendingTasksAction(null)}
					/>
				);
			case "overview":
				return (
					<div className="max-w-7xl mx-auto space-y-4">
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
							...
							<div className="rounded-lg bg-[var(--md-ref-color-primary-container)] p-3 text-center">
								<div className="text-lg font-semibold text-[var(--md-ref-color-on-primary-container)]">
									{taskStore.getTasksByState("RUNNING").length}
								</div>
							...
							<div className="rounded-lg bg-[var(--md-ref-color-surface-container-high)] p-3 text-center">
								<div className="text-lg font-semibold">
									{taskStore.getTasksByState("DONE").length}
								</div>
						...
						<div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
							{/* Timeline - 2 columns */}
							<div
								className="xl:col-span-2 flex flex-col rounded-lg bg-[var(--md-ref-color-surface-container-lowest)] border border-[var(--md-ref-color-outline-variant)]"
								style={{ minHeight: 400 }}
							>
			...
			case "life":
				return (
					<div className="h-full overflow-y-auto scrollbar-stable-y p-4">
						<RecurringTaskEditor
							action={recurringAction?.action}
							actionNonce={recurringAction?.nonce}
						/>
					</div>
				);
			case "settings":
