...
): { project: string; count: number; focusTime: number }[] => {
	const focusSessions = sessions.filter((s) => s.type === "focus" || s.type === "work");
...
function BarChart({
	...
	return (
		<div className="flex items-end justify-between gap-1 w-full" style={{ height }}>
			{data.map((d, i) => (
				<div key={`${d.label}-${i}`} className="flex-1 flex flex-col items-center gap-1 h-full">
					<div className="flex-1 w-full flex items-end">
						<div
							className={`w-full rounded-t-sm transition-all ${
								d.highlight ? "bg-blue-500" : isDark ? "bg-gray-700" : "bg-gray-300"
							}`}
							style={{
...
// Session list item
function SessionItem({ session, theme }: { session: PomodoroSession; theme: "light" | "dark" }) {
	const isDark = theme === "dark";
	...
	return (
		...
		>
			<div className="flex items-center gap-3">
				<div className={`w-2 h-2 rounded-full ${isFocus ? "bg-blue-500" : "bg-green-500"}`} />
				<div>
...
export default function StatsView() {
	...
	const localStats = useMemo(() => {
		...
		const todaySessions = sessions.filter(
			(s) =>
				s.endTime?.startsWith(todayStr) ||
				new Date(s.completedAt).toISOString().startsWith(todayStr),
		);
						...
						{activeTab === "all" && (
							<div className="space-y-4">
								...
								<div className={`rounded-xl p-4 ${isDark ? "bg-gray-800" : "bg-gray-50"}`}>
									<div className="flex items-center gap-3">
										<div
											className={`p-2 rounded-lg ${isDark ? "bg-yellow-500/20 text-yellow-400" : "bg-yellow-100 text-yellow-600"}`}
										>
