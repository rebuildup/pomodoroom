...
export default function SettingsView({ windowLabel }: SettingsViewProps = {}) {
	...
	const [pressureHistoryCount, setPressureHistoryCount] = useState(
		() => getPressureThresholdHistory().length,
	);
	...
	const [breakActivityCatalog, setBreakActivityCatalog] =
		useState<BreakActivity[]>(getBreakActivityCatalog);
			...
			<div className="max-w-7xl mx-auto space-y-6">
				...
				<section>
					...
					<div className="space-y-4">
						...
						<Button variant="tonal" size="small" onClick={() => setNudgeMetrics(getNudgeMetrics())}>
							メトリクス更新
						</Button>
					</div>
				</section>
				...
				<section>
					...
					<Button variant="tonal" size="small" onClick={resetBindings} className="mt-3">
						デフォルトに戻す
					</Button>
				</section>
				...
				<section className="pb-6">
					...
					<p className="text-xs leading-relaxed text-(--md-ref-color-on-surface-variant)">
						...
						各フォーカス期間の間に短い休憩、最後に長い休憩が入ります。 合計サイクル:{" "}
	...
	return (
		<section>
			...
			<div className="space-y-3">
				<ToggleRow
					label="タスクを全削除"
					value={options.deleteTasks}
					onChange={() => toggle("deleteTasks")}
				/>
				<ToggleRow
				...
				<ToggleRow
					label="グループを全削除"
					value={options.deleteGroups}
					onChange={() => toggle("deleteGroups")}
				/>
				<ToggleRow
				...
				{resultText && <p className="text-xs text-[var(--md-ref-color-primary)]">{resultText}</p>}
				{errorText && <p className="text-xs text-[var(--md-ref-color-error)]">{errorText}</p>}
			</div>
		</section>
	);
}
