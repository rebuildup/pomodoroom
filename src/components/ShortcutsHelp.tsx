...
export function ShortcutsHelp({ isOpen, onClose }: ShortcutsHelpProps) {
	...
	const byCategory = DEFAULT_SHORTCUTS.reduce(
		(acc, shortcut) => {
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center">
			...
			>
				...
				<div className="p-5 max-h-[60vh] overflow-y-auto">
					{Object.entries(byCategory).map(([category, shortcuts]) => (
						<div key={category} className="mb-6 last:mb-0">
							<h3 className="text-xs font-bold uppercase tracking-wider mb-3 text-[var(--md-ref-color-on-surface-variant)]">
							...
							<div className="space-y-2">
								{shortcuts.map((shortcut) => (
									<div key={shortcut.id} className="flex items-center justify-between text-sm">
										...
										<kbd className="px-2 py-1 rounded text-xs font-mono bg-[var(--md-ref-color-surface-container-highest)] border border-[var(--md-ref-color-outline)] text-[var(--md-ref-color-on-surface)]">
				...
				{/* Footer hint */}
				<div className="px-5 py-3 border-t text-xs text-[var(--md-ref-color-on-surface-variant)] border-[var(--md-ref-color-outline-variant)]">
