...
const ReadyItem = React.memo(
	({ item, onAction, onClick, compact }: ReadyItemProps) => {
		...
		return (
			...
			>
				...
				{(isRoutine || isDeferred) && <StatusBadge status={item.status} compact={compact} />}
				...
				`.trim()}
					style={{
						font: compact
							...
							: "var(--md-sys-typescale-body-large)",
					}}
				>
					{item.title}
				</span>
...
export const TaskStreamItem: React.FC<TaskStreamItemProps> = React.memo(
	({ item, onAction, onClick, compact = false, className = "" }) => {
		...
		return (
			<div className={className}>
				<ReadyItem item={item} onAction={onAction} onClick={onClick} compact={compact} />
			</div>
		);
	},
	(prevProps, nextProps) => {
