...
const release = {
	...
	assets: [
		...
		},
		{
			...
			browser_download_url: "https://example.com/Pomodoroom_1.2.3_x64-setup.exe.sig",
			body: "windows-sig",
		},
		{
		...
		},
		{
			...
			browser_download_url: "https://example.com/Pomodoroom_aarch64.app.tar.gz",
		},
		{
			name: "Pomodoroom_aarch64.app.tar.gz.sig",
			browser_download_url: "https://example.com/Pomodoroom_aarch64.app.tar.gz.sig",
			body: "mac-arm-sig",
		},
		{
		...
		},
		{
			...
			browser_download_url: "https://example.com/Pomodoroom_1.2.3_amd64.AppImage.sig",
			body: "linux-sig",
		},
	],
};
...
test("builds updater manifest with signatures for all supported platforms", async () => {
	...
	assert.equal(latest.platforms["darwin-aarch64"].signature, "mac-arm-sig");
	assert.equal(latest.platforms["linux-x86_64"].signature, "linux-sig");
});
...
test("uses signed MSI when unsigned NSIS asset is also present", async () => {
	const mixedWindowsRelease = {
		...release,
		assets: [
			...release.assets.filter((asset) => asset.name !== "Pomodoroom_1.2.3_x64-setup.exe.sig"),
			{
				name: "Pomodoroom_1.2.3_x64_en-US.msi",
				browser_download_url: "https://example.com/Pomodoroom_1.2.3_x64_en-US.msi",
			},
			{
				name: "Pomodoroom_1.2.3_x64_en-US.msi.sig",
				browser_download_url: "https://example.com/Pomodoroom_1.2.3_x64_en-US.msi.sig",
	...
	assert.equal(latest.platforms["windows-x86_64-msi"].signature, "windows-msi-sig");
});
...
test("uses stable tag download base URL when provided", async () => {
	const latest = await buildLatestJson({
		...
		release,
		fetchText: async (asset) => asset.body ?? "",
		downloadBaseUrl: "https://github.com/rebuildup/pomodoroom/releases/download/v1.2.3",
	});
