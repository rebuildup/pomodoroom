...
function waitForChecks(prNumber, intervalSec, dryRun) {
	run("gh", ["pr", "checks", String(prNumber), "--watch", "--interval", String(intervalSec)], {
		dryRun,
	});
}
...
function ensureChecksClean(prNumber, dryRun) {
	if (dryRun) return;
	const result = run("gh", ["pr", "view", String(prNumber), "--json", "statusCheckRollup"], {
		capture: true,
	});
...
const agentId =
	config.agentId || process.env.AUTOPILOT_AGENT_ID || `${os.hostname()}-${process.pid}`;
...
if (flowCfg.runChecks) {
	const commands =
		Array.isArray(flowCfg.checkCommands) && flowCfg.checkCommands.length > 0
...
if (flowCfg.mergePr) {
	...
	if (flowCfg.waitForChecks !== false) {
		const interval = Number.isInteger(flowCfg.checkPollIntervalSec)
			? flowCfg.checkPollIntervalSec
			: 15;
